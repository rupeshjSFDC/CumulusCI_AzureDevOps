...
class ClientToolClient(Client):
    ...
    def get_clienttool_release(
        ...
        if tool_name is not None:
            route_values["toolName"] = self._serialize.url(
                "tool_name", tool_name, "str"
            )
        query_parameters = {}
        if os_name is not None:
            query_parameters["osName"] = self._serialize.query(
                "os_name", os_name, "str"
            )
        if arch is not None:
            query_parameters["arch"] = self._serialize.query("arch", arch, "str")
        if distro_name is not None:
            query_parameters["distroName"] = self._serialize.query(
                "distro_name", distro_name, "str"
            )
        if distro_version is not None:
            query_parameters["distroVersion"] = self._serialize.query(
                "distro_version", distro_version, "str"
            )
        if version is not None:
            query_parameters["version"] = self._serialize.query(
                "version", version, "str"
            )
        response = self._send(
            http_method="GET",
