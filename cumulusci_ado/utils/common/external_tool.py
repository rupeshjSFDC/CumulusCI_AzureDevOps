...
class ExternalToolInvoker:
    ...
    def __init__(self):
        signal.signal(signal.SIGINT, functools.partial(self._sigint_handler))
        self._args = None
    ...
    def start(self, command_args, env):
        ...
        DEVNULL = open(
            os.devnull, "w"
        )  # Note: subprocess.DEVNULL not available on python 2.7
    ...
    def wait(self):
        ...
        if self._proc.returncode != 0 and not self._terminating:
            ...
            raise CumulusCIFailure(
                "Process {proc} with PID {pid} exited with return code {code}{err}".format(
                    proc=self._args,
                    pid=self._proc.pid,
...
class ProgressReportingExternalToolInvoker(ExternalToolInvoker):
    ...
    def run(self, command_args, env, initial_progress_text, stderr_handler):
        with Spinner(  # pylint: disable=no-member
            label=initial_progress_text, total=100, stream=sys.stderr
        ) as self._spinner:
            self._spinner.step()
            ...
            if self._proc is None or self._proc.stderr is None:
                raise RuntimeError(
                    "Failed to start external process or capture stderr."
                )
            try:
