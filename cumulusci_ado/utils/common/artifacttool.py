...
class ArtifactToolInvoker:
    def __init__(self, client_tool, tool_invoker, artifacttool_updater):
        self.client_tool = client_tool
        self._tool_invoker = tool_invoker
        self._artifacttool_updater = artifacttool_updater

    def download_pipeline_artifact(
        self, organization, project, run_id, artifact_name, path
    ):
        args = [
            "pipelineartifact",
            "download",
    ...
    def run_artifacttool(self, organization, args, initial_progress_message):
        ...
        artifacttool_dir = self._artifacttool_updater.get_latest_artifacttool(
            self.client_tool
        )
        ...
        new_env[ARTIFACTTOOL_PAT_ENVKEY] = str(
            self.client_tool.config.credentials.password
        )

        # Run ArtifactTool
        command_args = [artifacttool_binary_path] + args
        proc = self._tool_invoker.run(
            command_args, new_env, initial_progress_message, _process_stderr
        )
        if proc:
            ...
            try:
                ...
                if output:
                    logger.warning(
                        "Failed to parse the output of ArtifactTool as JSON. The output was:\n %s",
                        output,
                    )
        return None
...
def _process_event(json_line, update_progress_callback):
    if (
        ...
        and "EventId" in json_line
        and "Name" in json_line["EventId"]
    ):
        ...
        if event_name == "ProcessingFiles":
            ...
            update_progress_callback(
                "Pre-upload processing: {}/{} files".format(
        ...
        if event_name == "Uploading":
            ...
            update_progress_callback(
                "Uploading: {}/{} bytes".format(uploaded_bytes, total_bytes), percent
            )
        ...
        if event_name == "Downloading":
            ...
            update_progress_callback(
                "Downloading: {}/{} bytes".format(downloaded_bytes, total_bytes),
                percent,
            )
