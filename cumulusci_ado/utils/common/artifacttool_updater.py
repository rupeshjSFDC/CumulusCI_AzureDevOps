...
AZ_DEVOPS_GLOBAL_CONFIG_DIR = UniversalConfig.default_cumulusci_dir()
ARTIFACTTOOL_OVERRIDE_PATH_ENVKEY = "AZURE_DEVOPS_EXT_ARTIFACTTOOL_OVERRIDE_PATH"
ARTIFACTTOOL_OVERRIDE_URL_ENVKEY = "AZURE_DEVOPS_EXT_ARTIFACTTOOL_OVERRIDE_URL"
ARTIFACTTOOL_OVERRIDE_VERSION_ENVKEY = "AZURE_DEVOPS_EXT_ARTIFACTTOOL_OVERRIDE_VERSION"
...
class ArtifactToolUpdater:
    def get_latest_artifacttool(self, organization):
        artifacttool_binary_override_path = os.environ.get(
            ARTIFACTTOOL_OVERRIDE_PATH_ENVKEY
        )
        if artifacttool_binary_override_path is not None:
            artifacttool_binary_path = artifacttool_binary_override_path
            logger.debug(
                "ArtifactTool path was overriden to '%s' due to environment variable %s",
                artifacttool_binary_path,
                ARTIFACTTOOL_OVERRIDE_PATH_ENVKEY,
            )
        else:
    ...
    def _get_artifacttool(self, organization):  # pylint: disable=no-self-use
        ...
        if artifacttool_override_url is not None:
            ...
            logger.debug(
                "ArtifactTool download URL is being overridden to '%s' (ID '%s')",
                release_uri,
                release_id,
            )
        else:
        ...
        logger.debug(
            "Checking if we already have ArtifactTool release '%s'", release_id
        )
        release_dir = _compute_release_dir(release_id)
        if os.path.exists(release_dir):
            logger.debug(
                "Not updating ArtifactTool because the current release already exists at '%s'",
                release_dir,
            )
            return release_dir
        ...
        logger.debug(
            "Updating to ArtifactTool release %s since it doesn't exist at %s",
            release_id,
            release_dir,
        )
...
def _update_artifacttool(uri, release_id):
    ...
    with Spinner(
        label="Downloading Universal Packages tooling ({})".format(release_id),
        total=100,
        stream=sys.stderr,
    ) as spinner:
        spinner.step()
        ...
        for chunk in response.iter_content(chunk_size=1024 * 512):
            if chunk:
                ...
                spinner.step(int(100 * float(bytes_so_far) / float(content_length)))
        ...
        try:
            ...
            if os.path.exists(release_dir):
                ...
                logger.debug(
                    "Moving downloaded ArtifactTool from %s to %s",
                    release_temp_dir,
                    release_dir,
                )
                ...
                for _ in range(retries - 1):
                    try:
                        ...
                        logger.debug(
                            "An error occurred while renaming the Universal Packages tooling: %s. Retrying...",
                            ex,
                        )
                        time.sleep(1)
                else:
            ...
            logger.error(
                "An error occurred while extracting the Universal Packages tooling: %s",
                ex,
            )
...
def _get_current_release(client, override_version):
    ...
    return (release.uri, _compute_id(release)) if release is not None else (None, None)
